...
if "vendor_matches" not in st.session_state: st.session_state.vendor_matches = []
...
with col_queue:
    ...
    if total_started > 0:
        st.progress(
            st.session_state.done_count / max(total_started, 1),
...
with col_review:
    ...
    if btn_vendor.button("🔗 Search Similar Vendor Terms", use_container_width=True):
        with st.spinner(f"Finding similar mapped vendor terms for '{vendor_term}'..."):
            try:
    ...
    # ── Vendor term match cards ─────────────────────────────────────────────
    if st.session_state.vendor_matches:
        st.markdown("#### Similar Vendor Terms (Already Mapped)")
        st.caption("These terms from other vendors are already mapped — adopt the same concept.")

        for i, m in enumerate(st.session_state.vendor_matches):
            score_pct   = int(m.get("score", 0) * 100)
            score_color = "🟢" if score_pct >= 80 else "🟡" if score_pct >= 60 else "🔴"

            with st.expander(
                f"{score_color} **{m['vendor_term']}** ({m['vendor']}) "
                f"→ {m['concept_name']} — similarity: {score_pct}%",
                expanded=(i == 0)
            ):
                c1, c2, c3 = st.columns(3)
                ...
                st.caption(
                    f"*{m['vendor']}* uses `{m['vendor_code']}` → "
                    f"already mapped to **{m['concept_name']}**"
                )

                btn_a, btn_b = st.columns(2)

                if btn_a.button(
                    "✅ Confirm same mapping",
                    key=f"vconfirm_{i}",
                    use_container_width=True,
                    type="primary"
                ):
                    try:
                        ...
                        st.session_state.last_action = (
                            f"✅ Confirmed: **{vendor_term}** → {result['concept']} "
                            f"(inherited from {m['vendor']}: {m['vendor_term']})"
                        )
                        st.session_state.queue = [
                            t for t in st.session_state.queue
                            if not (t["term"] == vendor_term and t["vendor"] == vendor_name)
